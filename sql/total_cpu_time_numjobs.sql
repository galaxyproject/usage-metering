...
select job.tool_id,SUM(runtime_cpu_combined.cpu_usage * runtime_cpu_combined.runtime_seconds) as total_cpu_time, 
COUNT(distinct id) as num_jobs
from job join runtime_cpu_combined on job.id = runtime_cpu_combined.job_id
group by job.tool_id
order by total_cpu_time desc,num_jobs desc;